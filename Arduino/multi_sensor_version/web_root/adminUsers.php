...
if(strlen($username) > 0 && $edit==0){
	if ($cnt==0)die("countvalue is 1 based");
	$sql = "Insert into humiusers(username,email,apikey,img,scnt) Values('$username','$email', '$apikey','$img','$cnt')";	
	...
	if($delete==1 && $euid > 0){
	    ...
	    if ($cnt==0)die("countvalue is 1 based");
		$sql = "Update humiusers set username='$username', email='$email', apikey='$apikey', img='$img', scnt='$cnt' where autoid=$euid";
...
<tr><td>#sensors:</td><td><input type=text name=cnt size=45 value='<?=$cnt?>'></td></tr>
<tr><td align=center colspan=2>
	...
	<?if($edit==1){?><input class=btn type=button value="New User" onclick='location="adminUsers.php"'><?}?>
</td></tr>
</table>
</form>
